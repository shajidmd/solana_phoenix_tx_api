...
pub async fn fetch_and_parse_transactions(client: &BlockingRpcClient, address: &str) -> Result<(), FetchError> {
    
    let pubkey = Pubkey::from_str(address).map_err(|_| FetchError::InvalidPubkey(address.to_string()))?;

    // Fetch signatures for the given address
    let signatures = client.get_signatures_for_address(&pubkey).map_err(|e| FetchError::FetchSignaturesError(format!("Error fetching signatures: {:?}", e)))?;
...
pub async fn parse_fills(signature: &Signature) -> Result<(), FetchError> {
    ...
    .map_err(|_| FetchError::RPCInitializationError)?;
    ...
    let sdk_client: SDKClient = SDKClient::new_from_ellipsis_client_with_all_markets(client).await
